...
export const automationRules: AutomationRule[] = [
  { id: 'ar1', name: 'Novo Lead → Criar Tarefa', description: 'Quando um novo lead é cadastrado, cria automaticamente uma tarefa de follow-up para o vendedor responsável', trigger: 'Novo lead cadastrado', condition: 'Status = Lead', action: 'Criar tarefa "Follow-up inicial" (prazo: 2 dias)', triggerType: 'lead_created', triggerConfig: {}, conditionType: 'always', conditionConfig: {}, actionType: 'create_task', actionConfig: { title: 'Follow-up inicial', dueDays: 2, priority: 'high', type: 'follow_up' }, enabled: true, executions: 47, lastTriggered: '2025-01-28 14:30' },
  { id: 'ar2', name: 'Lead Inativo → Alerta', description: 'Quando um lead fica sem interação por mais de 5 dias, gera um alerta visual e cria tarefa automática', trigger: 'Lead sem interação > 5 dias', condition: 'Status ≠ Fechado', action: 'Alerta visual + Criar tarefa "Reativar contato"', triggerType: 'inactivity_days', triggerConfig: { days: 5, entity: 'contact' }, conditionType: 'always', conditionConfig: {}, actionType: 'create_task', actionConfig: { title: 'Reativar contato', dueDays: 1, priority: 'medium', type: 'follow_up' }, enabled: true, executions: 23, lastTriggered: '2025-01-27 08:00' },
  { id: 'ar3', name: 'Mudança de Estágio → E-mail', description: 'Quando um deal muda para Proposta, envia automaticamente um e-mail template com a proposta comercial', trigger: 'Deal mudou de estágio', condition: 'Novo estágio = Proposta', action: 'Enviar e-mail template "Proposta Comercial"', triggerType: 'deal_stage_changed', triggerConfig: { toStage: 'proposta_enviada' }, conditionType: 'always', conditionConfig: {}, actionType: 'create_activity', actionConfig: { type: 'email', description: 'Proposta comercial enviada automaticamente' }, enabled: true, executions: 15, lastTriggered: '2025-01-26 11:00' },
  { id: 'ar4', name: 'Deal Fechado → Onboarding', description: 'Quando um deal é marcado como Fechado, cria automaticamente tarefas de onboarding para o cliente', trigger: 'Deal estágio = Fechado', condition: 'Valor > R$ 50.000', action: 'Criar 3 tarefas de onboarding + Notificar gestor', triggerType: 'deal_stage_changed', triggerConfig: { toStage: 'fechado_ganho' }, conditionType: 'value_greater_than', conditionConfig: { entity: 'deal', field: 'value', value: 50000 }, actionType: 'create_task', actionConfig: { title: 'Onboarding do cliente', dueDays: 3, priority: 'high', type: 'meeting' }, enabled: true, executions: 8, lastTriggered: '2025-01-29 09:00' },
  { id: 'ar5', name: 'Follow-up Pós Demo', description: 'Após uma reunião de demo, cria tarefa de follow-up para 3 dias depois', trigger: 'Tarefa tipo "Demo" concluída', condition: 'Deal em Qualificado ou Proposta', action: 'Criar tarefa "Follow-up pós demo" (prazo: 3 dias)', triggerType: 'task_completed', triggerConfig: {}, conditionType: 'always', conditionConfig: {}, actionType: 'create_task', actionConfig: { title: 'Follow-up pós demo', dueDays: 3, priority: 'medium', type: 'follow_up' }, enabled: true, executions: 12, lastTriggered: '2025-01-27 16:00' },
  { id: 'ar6', name: 'Reativação de Inativos', description: 'A cada 30 dias, verifica contatos inativos e cria tarefa para tentar reativação', trigger: 'Ciclo mensal (dia 1)', condition: 'Status = Inativo, última interação > 30 dias', action: 'Criar tarefa "Tentativa de reativação"', triggerType: 'inactivity_days', triggerConfig: { days: 30, entity: 'contact' }, conditionType: 'field_equals', conditionConfig: { entity: 'contact', field: 'status', value: 'inactive' }, actionType: 'create_task', actionConfig: { title: 'Tentativa de reativação', dueDays: 1, priority: 'low', type: 'follow_up' }, enabled: false, executions: 3, lastTriggered: '2025-01-01 08:00' },
];
